...
X ~ dnPhyloOrnsteinUhlenbeckREML(fbd_tree, alpha, theta, sigma2^0.5, rootStates=theta)
X.clamp(data)
########
# MCMC #
########
...
monitors.append( mnModel(filename="output/fossilvase_ouf.log", printgen=10) )
#monitors.append( mnExtNewick(filename="output/fossilvase_ou.trees", isNodeParameter=TRUE, printgen=10, separator=TAB, tree=fbd_tree) )
...
#trace = readTreeTrace("output/fossilvase_ou.trees")

# Summarize tree trace and save MCC tree to file
#mccTree(trace, file="output/fossiloumcc.nex" )
